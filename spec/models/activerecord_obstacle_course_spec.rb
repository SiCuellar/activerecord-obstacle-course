...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    users = User.joins(:items).where('order_items.item_id = ?', item_8.id).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    names = Order.find(Order.all.last.id).items.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    #  Solution goes here
    orders = User.find(2).orders.joins(:items).where('item_id = ?', item_4.id)
    # -----------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select("users.name, count(items.id) AS total_item_count")
                    .joins(:items)
                    .group(:id)
                    .order(name: :desc)
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.select("users.name AS user_name, order_items.order_id AS order_id, count(order_items.item_id) AS item_count")
    .joins(:order_items)
    .group("users.name, order_items.order_id")
    .order(name: :desc)
    # ---------------------------------------------------------------
